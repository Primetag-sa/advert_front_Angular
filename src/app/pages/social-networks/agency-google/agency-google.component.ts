...
@Component({
  selector: 'app-agency-google',
  templateUrl: './agency-google.component.html',
  styleUrl: './agency-google.component.scss'
})



export class AgencyGoogleComponent implements OnInit{
  ...
  constructor(private store:Store,private router: Router,private route: ActivatedRoute,private googleService:GoogleService,private datePipe: DatePipe) {
  ...
  ngOnInit() {
    ...
    this.store.select(selectEntityList('google-ads/customers')).subscribe((account:any  ) => {
      if(account.data)
  ...
  signInGoogle():void{
    window.location.href= `${environment.apiUrl}/auth/google-ads?url=${this.router.url}`;
  }
  signOutGoogle():void{
    this.googleService.signOutGoogle().subscribe(
      (data: any) => {
  ...
  getAllAccounts(): void {
    this.accounts=this.userConneted?.customers_google?JSON.parse(this.userConneted?.customers_google):[];
  }
  refreshAccounts(){
    this.store.dispatch(getAllNoPagination({entity:"google-ads/customers", params: []}))
  }
  //Select advertiser
  ...
  getCampaigns(advertiser_id:string){
    this.store.select(selectEntityList('google-ads/customers/'+advertiser_id+'/campaigns')).subscribe((data:any  ) => {
    ...
    this.store.dispatch(getAllNoPagination({entity:"google-ads/customers/"+advertiser_id+"/campaigns",
        params: {
