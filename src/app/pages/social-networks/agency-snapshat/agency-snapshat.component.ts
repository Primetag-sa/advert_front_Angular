...
@Component({
  selector: 'app-agency-snapshat',
  templateUrl: './agency-snapshat.component.html',
  styleUrls: ['./agency-snapshat.component.scss']
})
export class AgencySnapshatComponent implements OnInit {
  alertState = false;
  exist = false;
  snapchatUser: any;
  ...
  */
  ...
  fetchAccounts(): void {
    this.snapchatService.getAccounts().subscribe((data) => {
  ...
  fetchCampaigns(accountId: number): void {
    this.snapchatService.getCampaigns(accountId).subscribe((data) => {
      this.campaignsSelect = data;
    });
  }

  fetchAdsAccount(accountId: number): void {
    this.snapchatService.fetchAdsByAccount(accountId).subscribe((data) => {
      this.adsSelect = data;
      if (data.length > 0) {
  ...
  fetchAds(adSquadId: number): void {
    this.snapchatService.getAdsByAdSquad(adSquadId).subscribe((data) => {
  ...
  fetchAdSquads(campaignId: number): void {
    this.snapchatService.getAdSquads(campaignId).subscribe((data) => {
      this.adSquadsSelect = data;
    });
  }

  fetchAdsCampaign(campaignId: number): void {
    this.snapchatService.fetchAdsByCampaign(campaignId).subscribe((data) => {
      this.adsSelect = data;
    });
  }
